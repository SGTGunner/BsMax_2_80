...
macroscript character_to_abc_mcr
	Tooltip:"Character to ABC V3.1"
...
(
	...
	(
		if cls.turnMeshTypeCB.state == 2 do (
			make_mesh objs
		)

		if cls.turnMeshTypeCB.state == 3 do (
			make_poly objs
		)
	)
	...
	(
		...
		for char in caharacterList do (
			...
			if cls.autoFix.state do (
				fix_parenting_issues selection
			)
			...
			if cls.autoFix.state do (
				fix_unskin_meshes objs
			)

			if cls.bakeTransformCB.state do (
				bake_in_range objs
			)

			if cls.pointCacheCB.state do (
				cache_meshes objs
			)
